...
demo = {
    ...
    showSwal: function (type,userName,crawlerCode,ipAddress,status,url,refreshUrl) {
         if (type == 'serverSupport') {
             nCrawlerCode = crawlerCode.replace("<strong>","").replace("</strong>","")
             tWail = "";
             if(status==true){
                 tWail = '停用'
             }else{
             ...
             swal({
                ...
                $.ajax({
                    ...
                    type:"POST",
                    data:{
                        crawlerCode:nCrawlerCode,
                        ipAddress:ipAddress,
                        status:status,
                    },
                    dataType:"TEXT",
                    success:function(data){
                        swal("已更改!", "爬虫可用状态已修改.", "成功");
                    }
                });
                swal("已更改!", "爬虫可用状态已修改.", "成功");
            });

        } else if (type == 'severEdit') {
             nCrawlerCode = crawlerCode.replace("<strong>","").replace("</strong>","");
             nIpAddress = ipAddress.replace(/ /g, '');
             swal({
                 ...
                 '    <div class="col-sm-6">\n' +
                 '      <input type="hidden" value="'+nCrawlerCode+'" name="crawlerCode" />\n' +
                 '      <input type="hidden" value="'+status+'" name="status" />\n' +
                 '      <input type="hidden" value="'+nIpAddress+'" name="sIpAddress" />\n' +
                 '      <input type="hidden" value="'+userName+'" name="userName" />\n' +
                 '      <input type="text" value="'+nIpAddress+'" class="form-control" name="ipAddress" placeholder="请输入ip地址"/>\n' +
                 '    </div>\n' +
                 '  </div>\n' +
                 '  <div class="form-group">\n' +
                 '    <div class="col-sm-offset-1 col-sm-10">\n' +
                 '      <button type="submit" class="btn btn-info btn-fill">提交修改</button>\n' +
                 '    </div>\n' +
                 '  </div>\n' +
                 '</form>\n'
             ...
             swal({
                 ...
                 swal({
                     ...
                     title:"ip爬虫支持状态已更新！",
                     html:
                     '<a href="'+refreshUrl+'" style="font-size: large">OK</a>'
                 });
            ...
            swal({
                    ...
                    html: '<p>1<input id="input-field" class="form-control">' +
                    '<p>1<input id="input-field1" class="form-control">' +
                    '<p>2<input id="input-field2" class="form-control">' +
                    '<p>3<input id="input-field3" class="form-control">',
                    showCancelButton: true,
                    closeOnConfirm: false,
